...
export function getAllFilesFrontMatter(folder: string = 'docs'): FrontMatter[] {
  return Sentry.startSpan({name: `getAllFilesFrontMatter`, op: 'function'}, () => {
...
// ESLINT reports a false postive here
// eslint-disable-next-line require-await
export async function getFileBySlug(slug) {
  return Sentry.startSpan({name: `getFileBySlug`, op: 'function'}, async () => {
    ...
    if (
      ...
      [mdxPath, mdxIndexPath, mdPath, mdIndexPath].filter(p => fs.existsSync(p))
        .length === 0
    ) {
